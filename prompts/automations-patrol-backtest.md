...
- Dedupe: do NOT dedupe against production issues; a replay is meant to reproduce the full result. State on each issue, in one line: `Dedupe: not applied (backtest replay)` — do not reuse the production `Already tracked:` prefix, which means the opposite.
