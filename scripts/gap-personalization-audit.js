#!/usr/bin/env node
/**
 * Gap offer personalization coverage audit.
 *
 * Scores the live offer pool for every Good Rewards tier the storefront can send and
 ...
 *
 * Usage: node scripts/gap-personalization-audit.js [--json]
 *        (or npm run audit:gap)
 */
...
const spec = require('../pipelines/gap/style-affinity-spec.json');
...
function storefrontTiers() {
  return Object.keys(MEMBERSHIP_PROGRAM_CODES);
}
...
function main() {
  ...
  const lines = [`Offer personalization coverage — feature view @ ${STYLE_AFFINITY_VIEW.specVersion}`, ''];
  ...
  if (unpersonalized.size) {
    lines.push(
      ...
      + 'Add or populate the segment(s) in pipelines/gap/style-affinity-spec.json and rebuild.',
    );
  }
