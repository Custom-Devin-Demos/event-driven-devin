...
async function processOrder(orderData) {
  ...
  logger.info('Processing Kraft Heinz distributor order', {
    orderId,
    ...
    service: 'kraftheinz-distributor-orders',
  });
  ...
  try {
    ...
    incrementMetric('kraftheinz_order.success', { route: '/api/kraftheinz/order', region: orderData.region });
    recordTiming('kraftheinz_order.latency', duration, { route: '/api/kraftheinz/order' });
    ...
    const duration = Date.now() - startTime;
    incrementMetric('kraftheinz_order.failure', { route: '/api/kraftheinz/order', errorClass: error.name });
    recordTiming('kraftheinz_order.latency', duration, { route: '/api/kraftheinz/order', error: 'true' });
    logger.error('Kraft Heinz distributor order failed', {
      orderId,
      error: error.message,
    ...
    createSessionAndAlert({
      ...
      culprit: 'app/services/verticals/kraftheinz.js — selectFulfillmentHub',
      ...
      service: 'kraftheinz-distributor-orders',
      verticalLabel: 'Kraft Heinz Distributor Orders',
      customer: 'kraftheinz',
      tags: [
        { key: 'route', value: '/api/kraftheinz/order' },
        { key: 'service', value: 'kraftheinz-distributor-orders' },
      ],
      ...
      release: 'kraftheinz-orders@1.0.0',
      environment: process.env.DD_ENV || 'prod',
      triggeredRule: '',
    }).catch((alertError) => {
      logger.error('Failed to trigger Devin session from Kraft Heinz order error', { error: alertError.message });
    });
    throw error;
  }
}
