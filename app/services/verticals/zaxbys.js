...
async function computeTierStatus(memberId) {
  ...
  const tier = TIER_THRESHOLDS.find(t => netPoints >= t.minimum) || TIER_THRESHOLDS[TIER_THRESHOLDS.length - 1];
  return { name: tier.level, points: netPoints, bonus: tier.multiplier };
}
