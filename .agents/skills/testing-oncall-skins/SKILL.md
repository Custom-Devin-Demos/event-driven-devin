...
- `industrials-edge.js` generates cert material with `openssl` at boot (~1.3s after listen), then rotates the enrolled site leaves. Wait for three `Industrial edge client certificate loaded` log lines and the rotation lines before timing the first quote, or you will be timing keygen.
