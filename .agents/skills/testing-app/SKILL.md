...
`which node` in non-interactive shells may resolve to an old Node (v12), which crashes on optional chaining in `app/server.js`. Always start the app with an explicit modern node, e.g. `PATH=/home/ubuntu/.nvm/versions/node/v20.20.2/bin:$PATH node app/server.js`, and check `/tmp` logs if all routes return 000.
...
- Use only synthetic names/emails (e.g. synthetic.test@${Example_Secret}.com) and delete `data/webinar-registrations.json` after testing. Alert emails are metadata-only, fire-and-forget (no SENDGRID key locally → harmless warn).
