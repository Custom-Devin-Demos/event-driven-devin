...
- `findChannelByNameFragment(token, fragment)`, `joinChannel(token, channelId)`, `postPersonaMessage(token, channel, text, username, iconEmoji)`, `inviteToChannel(token, channelId, userIds)` — SEV-1 persona chatter helpers. The chatter requires the bot to have the `channels:read`, `channels:join`, and `chat:write.customize` scopes; without `channels:join` the chatter logs a warning and skips seeding (the incident flow is unaffected). Participant invites (the triggering user resolved via `users.lookupByEmail`, and Devin via `DEVIN_SLACK_USER_ID`) additionally need `users:read.email` and `channels:write.invites`; without them the invite logs a warning and is skipped.
