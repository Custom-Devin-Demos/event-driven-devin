...
**Vertical guard.** `scripts/vertical-guard.sh` (cron, every 5 min) GETs `/health` plus the demo pages the old guards watched (`GUARD_PATHS` to override) on `127.0.0.1:3000`. It **never builds an image**: it skips while `.deploy.lock` is held and for 15 min after the last deploy (`releases/CURRENT`), and on a non-200 it does `compose up -d --no-build` then `compose restart` for `checkout-api`, at most once per 10 min, posting to Slack if that does not recover. Missing vertical files are reported to Slack, not "repaired" — a redeploy owns the tree. Logs to `/home/ubuntu/vertical-guard.log`.
