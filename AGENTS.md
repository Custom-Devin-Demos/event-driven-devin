...
### BNY NEXEN digital asset custody scenario (9bfabd45, Vite/React, external repo)
...
The app plants a registry gap: all four demo accounts hold digital assets, but `digitalAssetCustodyRules` in `frontend/src/domain/digitalAssetCustody.ts` (and `DIGITAL_ASSET_CUSTODY_RULES` in the Java `CustodyFixtures`) only registers safekeeping arrangements for US and GB, so **Instruct digital asset deposit** on the Frankfurt (`8842933`, DE) or Paris (`8842951`, FR) account dereferences an absent rule and throws. US and GB return an instruction normally. The holdings screen shows the "deposit instruction unavailable" notice, and the client `POST`s to `/api/9bfabd45/error` with `source: nexen-custody/web` and `service: customer-9bfabd45-web`, plus `platform`, `screen`, `action`, `accountNumber`, `clientName`, `market` tags. `reportAppFailure` in `app/services/verticals/9bfabd45.js` raises the Slack alert and Devin session directly (Sentry capture + `digital_asset_deposit.failure` metric).

Identity: the client may forward `devinEmail` / `devinUserId` / `devinOrgId`, and the service resolves an email to an org member with `DEVIN_SERVICE_KEY_9BFABD45` when no user id was sent; with nothing from the client it falls back to the demo owner (Hannah Huh, `U0B2YAUPSHL`), who is also the `slackMemberId` on every card. `CUSTOMER_ALERT_IDENTITY` maps `customer-9bfabd45-web` to the NEXEN `APP_REMEDIATION_DIRECTIVE` (register the missing markets, replace the undefined dereference with explicit handling, add frontend and backend completeness tests, refresh the hosted build). Regression coverage lives in `tests/9bfabd45-error.test.js`.
