...
| **BlazingMQ Cluster Operations — partition rebalance** (unlisted — direct URL only) | `/bloomberg`, `/bd631c20` | `app/public/verticals/bd631c20.html` | `GET /api/bd631c20/cluster`, `POST /api/bd631c20/rebalance` | `app/services/verticals/bd631c20.js` |
...
**Customer-vertical alert cards (`postAlertToSlack`) name a real owner or nobody — never a made-up one.** The *On-Call* field is `onCallText()`: the `slackMemberId` the vertical passed to `createSessionAndAlert`, else the member resolved from `devinEmail`, else `slackMemberIdFallback`, else the opt-in `DEMO_ONCALL_SLACK_MEMBER_ID`, else `_Unassigned_` (`ONCALL_UNASSIGNED_TEXT`) with no @-mention. Owner and session identity always default to what is set on the hub homepage: the page reads `devinUserId` / `devinOrgId` / `devinEmail` from the hub's localStorage and forwards them with the triggering request, the service passes them through to `createSessionAndAlert`, and the on-call member is resolved from that `devinEmail`, so Slack and Devin agree on who owns the incident. Do not hard-code a `slackMemberId`, `devinUserId`, `devinOrgId`, or email in a vertical, and do not default to any named person. A fictional name in that field reads to the audience as a real teammate, which is why the persona fallback was removed from this path; do not reintroduce one.
...
## Legacy stock platform services (`stacks/d67c33e4`)

Two self-contained services behind the stock & inventory control tower demo: a
Node 12 WMS sync worker (`stacks/d67c33e4/node-stock-sync`) and a Python 3.7
Flask forecast service (`stacks/d67c33e4/py-demand-forecast`). They have their
own manifests, are deliberately pinned to end-of-life dependency majors whose
APIs the code still uses, and are **not** installed by the root `package.json`,
