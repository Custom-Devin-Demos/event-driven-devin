...
- [ ] Devin Incident settings: enable incidents with Slack channel prefix `sev-1-incident-` so Devin auto-joins each new incident channel.
- [ ] Deploy env: add `DD_INCIDENT_APP_KEY` to the host `.env`.
