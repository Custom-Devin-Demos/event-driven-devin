...
- Outage failures are modelled as queue jobs rather than a log rate (`cadence` on the log spec): one poisoned job is born per tick and fails 8 times ~112s apart before dead-lettering, and its identity is derived from the absolute clock plus the run ref, so backfilled history, live emission and a resumed process all compute the same ids — a job's redeliveries stay intact across the declaration seam and across a restart. 753 single-use job ids was a tell in IR-113, and so was a cursor warning that never joined a failure: specs sharing a cadence id share token values, so the outage warning names the tick its failures cite exactly as the precursor burst does.
- Mitigation drains rather than silences (IR-113's investigator declared the incident "still active" because failure logs kept flowing at the pre-mitigation rate after Sam's disable line). Disabling the workflow stops new poisoned jobs; the ones already queued exhaust their redeliveries over ~13 min, with `queue.jobs.redelivered`/`dead_lettered` tapering alongside. An instant cliff contradicts the maxReceive=8 redrive policy the personas describe — and the taper is what gives a persona something real to report while recovery happens.
